...
from app.video import process_video_job
from app.analysis.process import process_snap_analysis_job
from app.models import CutVideoInput, AnalyzeSnapInput, JobStatus
...
# ---------------------------- App ------------------------------------
app = FastAPI(title="Video Cutter API", version="1.1.0")
app.add_middleware(
    CORSMiddleware,
...
# ---------------------------- Helpers ---------------------------------
async def _run_and_track(job: Job, coro_factory):
    try:
...
async def cut_video(payload: CutVideoInput = Body(...)):
    ...
    job.task = asyncio.create_task(_run_and_track(job, runner))
    return JobStatus(job_id=job.id, done=False, error=None, last_event=None)


# NEW: Analyze Snap endpoint with identical job/progress semantics
@app.post("/v1/analyze-snap", response_model=JobStatus)
async def analyze_snap(payload: AnalyzeSnapInput = Body(...)):
    if not os.path.exists(payload.file_path):
        raise HTTPException(status_code=400, detail=f"File not found: {payload.file_path}")
    os.makedirs(payload.output_dir, exist_ok=True)
    ...
    async def runner():
        # The analyze job should use emit({...}) events like {"type": "progress", "pct": 0-100, ...}
        await process_snap_analysis_job(
            job_id=job.id,
            params=payload,
            emit=lambda ev: job_manager.broadcast(job.id, ev),
        )
...
# Status + Cancel for cut_video (kept for backward compatibility)
@app.get("/v1/cut_video/{job_id}", response_model=JobStatus)
async def get_cut_video_job(job_id: str):
...
async def cancel_cut_video_job(job_id: str):
    job = job_manager.get(job_id)
...
# NEW: Status + Cancel for analyze-snap
@app.get("/v1/analyze-snap/{job_id}", response_model=JobStatus)
async def get_analyze_snap_job(job_id: str):
    job = job_manager.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    return JobStatus(job_id=job.id, done=job.done, error=job.error, last_event=job.last_event)
...
@app.post("/v1/analyze-snap/{job_id}/cancel")
async def cancel_analyze_snap_job(job_id: str):
    job = job_manager.get(job_id)
    if not job or not job.task:
        raise HTTPException(status_code=404, detail="Job not found or not running")
    job.task.cancel()
