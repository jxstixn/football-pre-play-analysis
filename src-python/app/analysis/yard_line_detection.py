...
def extend_and_cluster(yard_lines, img_height, eps=20, min_samples=1):
    ...
    for mid_x in mid_xs_sorted:
        if mid_x - last_mid_x > padding_threshold:
            idx = mid_xs.index(mid_x)
            filtered_masters.append(masters[idx])
            last_mid_x = mid_x

    return filtered_masters


def detect_yard_lines(img_path: str, output_img_path: str, max_degree: int = 60):
    ...
    if lines_p is not None:
        for x1, y1, x2, y2 in lines_p[:, 0]:
            ...
            if abs(angle - np.pi / 2) < np.deg2rad(max_degree):  # within ±
                yard_lines.append((x1, y1, x2, y2))

    # Step 6: Extend and cluster lines
    H, W = img.shape[:2]
    final_yard_lines = extend_and_cluster(yard_lines, H, eps=50, min_samples=2)

    output_img = img.copy()

    for x_top, x_bot in final_yard_lines:
